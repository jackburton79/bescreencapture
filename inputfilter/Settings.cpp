...
Settings::Load()
{
	...
	if (status == B_OK) {
		...
		if (tempMessage.FindBool(kQuitWhenFinished, &boolean) == B_OK)
			fSettings->SetBool(kQuitWhenFinished, boolean);
...
{
	BAutolock _(fLocker);
	fSettings->SetBool(kQuitWhenFinished, quit);
}


bool
Settings::QuitWhenFinished() const
{
	BAutolock _(fLocker);
	bool quit = true;
	fSettings->FindBool(kQuitWhenFinished, &quit);
	return quit;
}
...
Settings::_SetDefaults()
{
	...
	fSettings->MakeEmpty();
	fSettings->SetRect(kCaptureRect, rect);
	fSettings->SetString(kOutputFile, "/boot/home/clip.mpg");
